...
Network_Plot::Network_Plot(wxNotebookPage *page, size_t number_of_cpus, std::vector<wxColour> cpu_colors) : Generic_Plot(page){
	rx_plot = new mpFXYVector();
	rx_plot->SetContinuity(true);
	rx_plot->SetDrawOutsideMargins(false);
	rx_plot->SetData(x_points, vector<double>);
	...
	tx_plot->SetData(x_points, vector<double>);
...
void Network_Plot::add_point(double x, std::map<std::string, std::vector<double>> y){
    x_points.push_back(x);
    for (auto item : y) {
        y_points_rx[item.first].push_back(item.second.get_rx());
        y_points_tx[item.first].push_back(item.second.get_tx());
    }
}
...
void Network_Plot::update_gui(std::string interface){
    if(interface != ""){
        ...
        network_plot_window->Fit(double(x_points.back()- PLOT_SIZE / 2), double(x_points.back()), min_y - 5, max_y + 5);
    }
}